						...
						<c:forEach items="${item.skuList }" var="sku" varStatus="state">
							<c:choose>
								<c:when test="${state.index == 0 }">
									<a href="javascript:void(0);"  class="here" skuId="${sku.skuId }" skuPrice="${sku.skuPrice }">
								...
								<c:otherwise>
									<a href="javascript:void(0);"  skuId="${sku.skuId }" skuPrice="${sku.skuPrice }">
										<c:forEach items="${sku.specList }" var="spec">
											${spec.specValue }
										</c:forEach>
									</a>
