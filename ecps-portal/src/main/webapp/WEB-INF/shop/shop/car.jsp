...
function changeNum(skuId, quantity){
	if(quantity == 0){
		if(confirm("确认要删除该购物车的数据吗?")){
			window.location.href = "${path}/cart/deleteCart.do?skuId="+skuId;
		}
	}else{
		var result = validSkuStock(skuId, quantity);
		if(result.sku.stockInventory < quantity){
			alert("当前商品库存不足"+quantity+"个仅有"+result.sku.stockInventory+"个");
			window.location.href = "${path}/cart/modifyCart.do?skuId="+skuId+"&modQuantity="+result.sku.stockInventory;
		}else{
			...
			<tbody>
				...
				<c:forEach items="${cartList }" var="cart">
					...
					<td class="nwp pic">
						<ul class="uls">
							<li>
								<a href="${path }/html/${cart.sku.item.itemId}.html" title="" class="pic"><img src="${request_file_path }${cart.sku.item.imgs}" alt="摩托罗拉ME525" /></a>
								<dl>
									<dt><a href="${path }/html/${cart.sku.item.itemId}.html">${cart.sku.item.itemName }</a></dt>
								</dl>
							</li>
					...
					<td>
						<a href="javascript:void(0);" title="减" class="inb arr" onclick="changeNum(${cart.skuId}, ${cart.quantity - 1 })">-</a>
						<input type="text" class="txts" size="1" name="" value="${cart.quantity }" onblur="changeNum(${cart.skuId}, this.value)"/>
						<a href="javascript:void(0);" title="加" class="inb arr" onclick="changeNum(${cart.skuId}, ${cart.quantity + 1 })">+</a></td>
...
<div id="loginAlert" class="alt login" style="display:none">
	...
	<div class="cont">
		...
		<li>
			...
			<img alt="换一张" id="capthcaImage" class="code" onclick="changeImage();" src="${path }/user/getImage.do" /><a href="#" onclick="changeImage();" title="换一张">换一张</a>
		</li>
		<li class="gray"><label>&nbsp;</label><input type="checkbox" name="">记住我的手机号码</li>
		<li><label>&nbsp;</label><input type="button" id="loginSubmit" class="hand btn66x23" value="登 录" onclick="ajaxLogin()" ><a title="忘记密码？" href="/ecps-portal/ecps/portal/getpwd/getpwd1.do">忘记密码？</a></li>
