...
<form action="${path }/order/completeTask.do" method="post" id="completeForm">
	<input name="orderId" value="${tb.order.orderId }" type="hidden">
	<input name="taskId" value="${tb.task.id }" type="hidden">
	<input id="outcome" name="outcome" type="hidden">
				...
				<form id="myForm" action="${path}/order/orderPay/oper.do"
					...
					<div class="sch page_c">
						<span class="l">订单号：<b class="red"><var>${tb.order.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${tb.order.orderTime}" pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"></b>&nbsp;&nbsp;&nbsp; 
							...
							<c:forEach items="${tb.outcome }" var="outcome">
								<input name="r" type="button"  class="pointer sub1 red" value="${outcome }" />
							</c:forEach>
...
<div id="tab_2" style="display: none">
	<table cellspacing="0" summary="" class="tab3">
		...
		<tr><th width="12%">付款时间：</th><td><var><fmt:formatDate value="${tb.order.payTime}" pattern="yyyy-MM-dd HH:mm:ss" /></var></td></tr>
		<tr><th width="12%">到账时间：</th><td><var><fmt:formatDate value="${tb.order.depositTime}" pattern="yyyy-MM-dd" /></var></td></tr>
	</table>
</div>		
